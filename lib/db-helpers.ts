...
export async function getStudentInfo(userId: string) {
   ...
   const { data } = await supabase
      .from('student_profiles')
      .select(
         '*, batches(batch_name, fee_amount, fee_deadline, late_fee_percentage)',
      )
      .eq('id', userId)
      .single();
   return data;
...
export async function getPermissionSlips(studentId: string) {
   ...
   // First get approved submissions for this student
   ...
   // Get permission slips for approved submissions only
