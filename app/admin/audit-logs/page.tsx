...
export default async function AuditLogsPage() {
  ...
  const targetIds = Array.from(
    new Set((logs || []).map((log: any) => getLogTargetId(log)).filter((id: any) => !!id)),
  )
  ...
  const rawStudentIds = Array.from(
    new Set((submissions || []).map((s: any) => s?.student_id).filter((id: any) => !!id)),
  )
  ...
  const studentsById = new Map((studentsFallback || []).map((s: any) => [s.id, s]))
  const studentsByUserId = new Map((studentsFallback || []).map((s: any) => [s.user_id, s]))
  ...
  const missingSubmissionIds = targetIds.filter((id: any) => !submissionById.has(id))
  ...
  const enrichedLogs = (logs || []).map((log: any) => {
    ...
    const resolvedStudent =
      submission?.students ||
      (submission?.student_id ? studentsById.get(submission.student_id) : null) ||
      (submission?.student_id ? studentsByUserId.get(submission.student_id) : null)
    ...
    return {
      ...
      submission_reference: submission
        ? {
            ...
            registration_number: resolvedStudent?.registration_number || null,
            student_email:
              (resolvedUserId ? studentEmailMap.get(resolvedUserId) || null : null) ||
              fallbackEmail,
          }
        : null,
