...
async function generatePermissionSlipPdfBytes(input: {
   ...
   drawLabel(
      'Issued At:',
      new Date(input.issuedAt).toLocaleString(),
      startY - 182,
   );
   ...
   page.drawText(
      'Please present this slip along with your student ID card at the exam hall.',
      {
         x: 60,
         y: startY - 285,
         size: 11,
...
export async function POST(req: NextRequest) {
   ...
   // Get approved submissions for this student first
   ...
   // Only fetch slips linked to approved submissions if we have any
   const { data: directSlip } =
   ...
   const { data: studentRow } = await supabaseAdmin
      .from('students')
      .select(
         'id, registration_number, name, user_id, batch_id, current_semester',
      )
      .eq('user_id', user.id)
      .maybeSingle();
   ...
   console.log('DEBUG: Student info', {
      userId: user.id,
   ...
   // Check ALL fee_submissions with these student IDs
   const { data: allFeeSubmissions } = await supabaseAdmin
      .from('fee_submissions')
      .select('id, student_id, status, created_at')
      .in('student_id', possibleStudentIds)
      .order('created_at', { ascending: false });

   console.log('DEBUG: All fee submissions for possible student IDs', {
      count: allFeeSubmissions?.length || 0,
      statuses: allFeeSubmissions?.map((s: any) => s.status) || [],
      data: allFeeSubmissions,
   });

   // First get approved submissions for possible student IDs
   const { data: approvedSubmissionsForStudent } = await supabaseAdmin
      .from('fee_submissions')
      .select('id, student_id, status')
      .in('student_id', possibleStudentIds)
      .ilike('status', 'approved');

   console.log('DEBUG: Approved submissions', {
      count: approvedSubmissionsForStudent?.length || 0,
      data: approvedSubmissionsForStudent,
   });

   const approvedSubmissionIdsForStudent = (
      approvedSubmissionsForStudent || []
   ).map((s) => s.id);

   // Only fetch slips linked to approved submissions
   const { data: existingSlip } =
      approvedSubmissionIdsForStudent.length > 0
         ? await supabaseAdmin
              .from('permission_slips')
              .select('*')
              .in('student_id', possibleStudentIds)
              .in('submission_id', approvedSubmissionIdsForStudent)
              .order('generated_at', { ascending: false })
              .maybeSingle()
         : { data: null };
   ...
   if (!submissions || submissions.length === 0) {
      ...
      console.log('DEBUG: No approved submissions found', {
         userId: user.id,
         possibleStudentIds,
         approvedCount: approvedCount || 0,
         allSubmissionsCount: allFeeSubmissions?.length || 0,
      });
      ...
      return NextResponse.json({
         ...
         debug: {
            ...
            allSubmissionsCount: allFeeSubmissions?.length || 0,
            submissionStatuses:
               allFeeSubmissions?.map((s: any) => s.status) || [],
         },
      });
   }
   ...
   const existingBySubmission = new Set(
      (existingSlips || []).map((s: any) => s.submission_id),
   );
   ...
   for (const submission of missing) {
      ...
      const pdfBytes = await generatePermissionSlipPdfBytes({
         ...
         semester: String(
            batchRow?.semester || studentRow?.current_semester || 'N/A',
         ),
   ...
   const allSlips = [...(existingSlips || []), ...slipsCreated].sort(
      (a: any, b: any) => {
